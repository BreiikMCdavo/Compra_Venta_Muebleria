...
@section('content')
    ...
    <body>
        ...
        @php
            // Solo muestra productos que tienen imagen Y están activos (estado = 1)
            $productos = \App\Models\Producto::whereNotNull('img_path')->where('estado', 1)->get();
        @endphp
        ...
        @if($productos->count())
            <div class="galeria">
                @foreach($productos as $producto)
                    <div class="cuadro">
                        <img src="{{ asset('storage/productos/' . $producto->img_path) }}" alt="{{ $producto->nombre }}">
                        <div class="descripcion">{{ $producto->nombre }}</div>
                    </div>
                @endforeach
            </div>
        @else
            <p class="text-muted mt-2" style="text-align:center;">No hay productos con imagen disponible</p>
        @endif
